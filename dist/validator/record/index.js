...
class RecordValidator extends base_1.BaseValidator {
    ...
    async validateHtml(data) {
        const results = await html_validator_1.default({
            data,
            format: 'json'
        });
        return results.messages;
    }
    async checkHttp(value) {
    ...
    async reverseLookup(value) {
        const result = [];
        try {
            for (const record of this.records) {
                const domains = await dns_1.Dns.reverse(record);
                let valid = true;
                if (value) {
    ...
    async resolve() {
        try {
            if (this.type) {
                return await dns_1.Dns.resolve(this.name, this.type);
            }
